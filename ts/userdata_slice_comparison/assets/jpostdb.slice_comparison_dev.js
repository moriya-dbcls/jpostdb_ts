...
//	jpostdb.httpReq("get", url, null, group.stats_table, svg, renderDiv, param.yAxisX, param.top);
	...
	var textSearch = function(keyword){
		...
		if( proteinList[i].name.toLowerCase().match(keyword)
		    || proteinList[i].id.toLowerCase().match(keyword)
		...
		for(var i = 0; i < excludeList.length; i++){
		    if( excludeList[i].name.toLowerCase().match(keyword)
			|| excludeList[i].id.toLowerCase().match(keyword)
	...
	var plot = function(f){
		...
		for(var i = 0; i < proteinList.length;i++){
		    ...
		    if(proteinList[i].gene) genes.push(proteinList[i].gene);
		}
